...
def config_parser():
    ...
    parser.add_argument('--config', 
                        help='config file path',type=str,default='configs/pool.py')
...
def train(args, cfg, data_dict):
    ...
    # fit model geometry reconstruction
    eps_fine = time.time()
    if cfg.coarse_train.N_iters == 0:
        ...
        # xyz_center = torch.tensor([(xyz_max_fine[0] + xyz_min_fine[0]) / 2 , (xyz_max_fine[1] + xyz_min_fine[1]) / 2,(xyz_max_fine[2] + xyz_min_fine[2]) / 2])
        # box_r = (xyz_max_fine - xyz_min_fine).norm() / 2
        # xyz_max_fine_n = xyz_center.add(box_r)
        # xyz_min_fine_n = xyz_center.add(-box_r)
        # from IPython import embed;embed()
        xyz_r = xyz_max_fine - xyz_min_fine
        xyz_r_index = xyz_r.argmax()
        for i in range(3):
            if i != xyz_r_index:
                ax_v = xyz_r[xyz_r_index] - xyz_r[i]
                xyz_max_fine[i] = xyz_max_fine[i] + ax_v / 2
                xyz_min_fine[i] = xyz_min_fine[i] - ax_v / 2
         
        xyz_center = torch.tensor([(xyz_max_fine[0] + xyz_min_fine[0]) / 2 , (xyz_max_fine[1] + xyz_min_fine[1]) / 2 , (xyz_max_fine[2] + xyz_min_fine[2]) / 2])
        xyz_max_fine = xyz_max_fine - xyz_center
        xyz_min_fine = xyz_min_fine - xyz_center

        xyz_th = 0.05
        xyz_max_fine = xyz_max_fine - xyz_th
        xyz_min_fine = xyz_min_fine + xyz_th

        xyz_max_fine = xyz_max_fine + xyz_center
        xyz_min_fine = xyz_min_fine + xyz_center

        # from IPython import embed;embed()
        # r_a = torch.deg2rad(torch.tensor([30])) 
        # rot_mat = torch.tensor([[torch.cos(r_a),-torch.sin(r_a),0],[torch.sin(r_a),torch.cos(r_a),0],[0,0,1]])
        # # points = torch.tensor([xyz_max_fine,xyz_min_fine])
        # rot_mat.to(dtype=float)
        # xyz_max_fine_n = torch.matmul(rot_mat,xyz_max_fine - xyz_center )
        # xyz_max_fine_n = xyz_max_fine_n + xyz_center 
        

        # xyz_min_fine_n = torch.matmul(rot_mat,xyz_min_fine - xyz_center )
        # xyz_min_fine_n = xyz_min_fine_n + xyz_center  
...
def object_reconstuction(args,cfg,data_dict):
    ...
    if args.export_fine_only:
        ...
        export_fine_only_dir = os.path.join(cfg.basedir, cfg.expname, args.export_fine_only)
        np.savez_compressed(export_fine_only_dir, alpha=alpha, rgb=rgb)
        ...
        print('Active rate', (alpha > thres).mean())
        print('Active nums', (alpha > thres).sum())
        ...
        binvox_volume = tools.utils.binvox_rw.Voxels(volume > thres,dims,translate,scale,'xyz')

        # volume_save_path = os.path.join(args.path, 'GT-voxels-%06d.binvox' )
        volume_save_path = os.path.basename(export_fine_only_dir) + '.binvox'
        volume_save_path = os.path.join(cfg.basedir, cfg.expname, volume_save_path)
...
def run_btach():
    ...
    # class_name = '03001627_1'
    # class_name = '04379243_1'
    # class_name = '02691156_1'
    # class_name = '03636649_1'
    # class_name = '02828884_1'
    # class_name = '02933112_1'
    # class_name = '03211117_1'
    # class_name = '04256520_1'
    # class_name = '02958343_1'
    # class_name = "04090263_1"
    # class_name = "04401088_1"
    # class_name = "04530566_1"
    class_name = "03691459_1"
    cfg.basedir = '../myShapeNet_render/' + class_name
    # cfg.basedir = '../myShapeNet_render_800res/' + class_name
    

    expname_list = os.listdir(cfg.basedir)
    # from IPython import embed;embed()

    for i, expname in enumerate(expname_list):
        cfg.data['datadir'] = os.path.join(cfg.basedir,expname_list[i])
        cfg.expname = expname
        # load images / poses / camera settings / data split
        try:
        ...
        # data_dict = load_everything(args=args, cfg=cfg)
        try:
            object_reconstuction(args,cfg,data_dict)
        except IndexError:
            continue
